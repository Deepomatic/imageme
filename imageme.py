...
def _create_index_file(
    ...
    for page in range(0, num_pages):
        ...
        header_text = \
            'imageMe: ' + location + ' [' + str(len(image_files)) + ' image(s)] - Page ' + str(page)
        html = [
            '<!DOCTYPE html>',
            '<html>',
            '    <head>',
            ...
            '				-webkit-filter: grayscale(10%) brightness(30%);',
            ...
            '						$(".header-rm").append(e+" ");',
            '					});',
	 	 	'					$(document).scrollTop( $("#select-all").offset().top );',
			'				});',
        ...
        for directory in directories:
            link = directory + '/' + _get_index_file_name(page=0)
            html += [
                '    <h3 class="header">',
        ...
        html += [ '<div>']
        if page > 0:
        	html += ['	<a href="'+_get_index_file_name(page=page-1)+'">PREVIOUS PAGE</a>&nbsp;']

        for i in range(0, num_pages): 
        	if i < page or i > page:
        		html += ['	<a href="' + _get_index_file_name(page=i) + '">' + str(i) + '</a> &nbsp;']
        	else:
        ...
        if page < num_pages - 1:
        	...
        	html += ['&nbsp;&nbsp;	<a id="unselect-all">Unselect all images</a>&nbsp;']
        ...
        for image_file in image_files[page*args.num:(page+1)*args.num]:
            ...
            if table_row_count == args.numperrow:
                table_row_count = 0
                # html.append('</tr>')
            table_row_count += 1
        html += ['</ul>', '</div>']
        ...
        index_file_path = _get_index_file_path(location, page=page)
...
def _get_index_file_name(page=None):
	"""
	Get the file name to be used for an index file.
	"""
	if page is None:
		return INDEX_FILE_NAME
	return "{0}-{1}.html".format(INDEX_FILE_BASENAME, str(page))

def _get_index_file_path(location, page=None):
    """
    ...
    """
    return os.path.join(location, _get_index_file_name(page=page))
...
def _get_thumbnail_image_from_file(dir_path, image_file):
    ...
    scale_ratio = args.maxwidth / float(img_width)
    # Work out target image height based on the scale ratio
    target_height = int(scale_ratio * img_height)
    # Perform the resize
    try:
        img.thumbnail((args.maxwidth, target_height), resample=RESAMPLE)
    except IOError as exptn:
...
parser.add_argument("path", help="the path to the directory we want to analyse")
parser.add_argument("-p", "--port", type=int, help="the port the server will listen to")
parser.add_argument("-n", "--num", type=int, help="the max number of image per page")
parser.add_argument("-w", "--maxwidth", type=int, help="the max width of thumbnails")
parser.add_argument("-th", "--maxheight", type=int, help="the max height of thumbnails")
parser.add_argument("-r", "--numperrow", type=int, help="the max number of thumbnails per row")
...
if args.num is None:
	args.num = MAX_NUM_IMGS

if args.maxwidth is None:
	args.maxwidth = THUMBNAIL_WIDTH

if args.maxheight is None:
	args.maxheight = THUMBNAIL_HEIGHT

if args.numperrow is None:
	args.numperrow = IMAGES_PER_ROW
